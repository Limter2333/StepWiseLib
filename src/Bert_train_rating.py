...
from Model import FakeReviewClassifier,FakeReviewClassifier_rating
...
data = load_dataset_zip("")
X_train_index, X_test_index, y_train, y_test = train_test_split(data.index, data.label, train_size=0.8, random_state=42)
X_train = data.iloc[X_train_index].content
X_test = data.iloc[X_test_index].content

X_train_rating = data.iloc[X_train_index].rating
X_test_rating = data.iloc[X_test_index].rating
...
bert_clf = FakeReviewClassifier_rating(batch_size=BATCH_SIZE)
bert_clf.to(device)
...
train_dataset = TensorDataset(train_tokens_ids, train_masks_tensor, torch.tensor(X_train_rating.to_list()), train_y_tensor)
train_sampler = RandomSampler(train_dataset)
train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=BATCH_SIZE)

test_dataset = TensorDataset(test_tokens_ids, test_masks_tensor, torch.tensor(X_test_rating.to_list()),test_y_tensor)
...
for epoch_num in range(EPOCHS):
    ...
    for step_num, batch_data in enumerate(train_dataloader):
        token_ids, masks,ratings, labels = tuple(t.to(device) for t in batch_data)
        print(str(torch.cuda.memory_allocated(device)/1000000 ) + 'M')
        logits = bert_clf(token_ids,ratings ,masks)
